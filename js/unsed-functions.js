...
function getUserChannel() {
  ...
  request.execute(function(response) {
    ...
    if ('error' in response) {
      ...
      //getPlaylistItems(uploadsListId); //see function.js
    }
  });
}
...
function getPlaylistItems(listId) {
  ...
  var request = gapi.client.youtube.playlistItems.list({
    ...
    //https://www.youtube.com/playlist?list=UU6wTJp2FgN4NQF37E2y_OUQ
...
function getVideoMetadata(videoId, index) {
	...
	request.execute(function(response) {
		if ('error' in response) { return console.log(response.error.message) || false;
			...
			var CH_id = response.items[0].snippet.channelId;
			var PL_id = CH_id.replaceAt(1, "U"); //replace second character of channel ID
